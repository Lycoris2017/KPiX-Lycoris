...
bool Device::setXmlConfig( xmlNode *node ) {
   ...
   update = false;
   if (wmqInDev) cout<<"[Device:dev] I am setXmlConfig(node)"<<endl;
...
void Device::execXmlCommand ( xmlNode *node ) {
   ...
   for ( childNode = node->children; childNode; childNode = childNode->next ) {
      if ( childNode->type == XML_ELEMENT_NODE ) {
	if (wmqInDev) cout<<"[Device:dev] childnode==xml_element_node!"<<endl;
         // Extract name
         nodeName  = (const char *)childNode->name;
         ...
         if ( devMapIter != devices_.end() ) {
            dev = devMapIter->second;
	    if (wmqInDev)  cout<<"[Device:dev] Element is a device"<<endl;
            // Execute command
	   ...
	   if (wmqInDev) cout<<"[Device:dev] command() called w/ name == "<<nodeName<<"; value == "<< nodeValue <<endl;
	   command(nodeName,nodeValue);
	 }
      }
   }
}
...
void Device::command ( string name, string arg ) {
   ...
   if ( getInt("Enabled") == 0 ) {
     if (wmqInDev) cout<<"[Device:dev] device is not enabled"<<endl;
     return;
   }
   // Register read
...
void Device::writeConfig ( bool force ) {
   ...
   // Exit if not enabled
   if ( ! getInt("Enabled") ) return;
   if (wmqInDev) cout<< "[Device:dev] I will writeConfig() w/ force=="<<force<<endl;
   REGISTER_LOCK
...
void Device::hardReset () {
  //cout<<"[Device:dev] hardreset here!"<<endl;
