...
System::System ( string name, CommLink *commLink ) : Device(0,0,name,0,NULL) {
   ...
   addVariable(v = new Variable("DataCompress",Variable::Configuration));
   v->setDescription("Compress Data Files");
   v->setTrueFalse();
   v->setHidden(true);   
...
void *System::swRunStatic ( void *t ) {
   System *ti;
   ti = (System *)t;
   //ti->setDebug(true);// added by wmq
   if (wmqInSys)  cout<<"[System:dev] swRunStatic() here!"<<endl;
   ti->swRunThread();
   pthread_exit(NULL);
   return(NULL);
}
...
void System::command ( string name, string arg ) {
   ...
   if (wmqInSys) cout<<"[System::dev] command, name == "<<name<<endl;
   ...
   if ( name == "SetDefaults" ) {
      ...
      /*wmq dev @sept 25*/
      /*std::cout<<"[eudaq:dev] run count = "<< getVariable("RunCount")->getInt() <<std::endl;
      int mytest;
      cin >> mytest;
      getVariable("RunCount")->setInt(mytest);
      std::cout<<"[eudaq:dev] run count = "<< getVariable("RunCount")->getInt() <<std::endl;
      */
      //softReset();
   }
   ...
   // Open data file
      ...
      if (wmqInSys) cout<<"[System:dev] open datafile == \n    "<<getVariable("DataFile")->get()<<endl;
   ...
   // Close data file
     ...
     if ( get("DataOpen") == "True" ) {
       if (wmqInSys) cout<<"[System:dev] I am here for closedatafile"<<endl;
       readStatus();
       commLink_->addStatus(statusString(true,false,false,true));
...
bool System::parseXml ( string xml, bool force ) {
   ...
   if (wmqInSys) cout<<"[System:dev]: parseXml => "<<xml<<endl;
   // get unique xml id
   ...
   try {
      ...
      for ( childNode = node->children; childNode; childNode = childNode->next ) {
         if ( childNode->type == XML_ELEMENT_NODE ) {
	      ...
	      if (wmqInSys) cout<<"[system:dev] I am a config xml!"<<endl;
...
void System::parseXmlString ( string xml ) {
  if (wmqInSys)  cout<<"[system:dev] i am parseXmlString: "<<xml<<endl;
   try { 
...
void System::hardReset ( ) {
  //cout<<"[System:dev] hardreset here!"<<endl;
   errorFlag_ = false;
   configureFlag_ = false;
   configureMsg_ = "System Is Not Configured.\nSet Defaults Or Load Settings!\n";
   setRunState("Stopped");
...
string System::poll(ControlCmdMemory *cmem) {
   ...
   try {
	 ...
	 //std::cout<<"[dev] datarxcount=" <<msg.str() <<std::endl; // wmq
